...
const DEFAULT_OUTPUT = "apps/api/src/data/places/generated-indoor-plays.ts";
...
function canonicalName(value) {
  return normalizeName(value)
    .replace(/^Kid's US LAND$/i, "Kid's US.LAND")
    .replace(/^Kids US\.LAND$/i, "Kid's US.LAND")
    .replace(/\s+/g, " ")
    .trim();
}
...
function slugify(input) {
  ...
  return `osm-indoor-play-${hash.toString(16).padStart(8, "0")}`;
}
...
function hasIndoorEvidence(tags, name) {
  return (
    ...
    (tags.leisure === "playground" &&
      /^(yes|only|indoor)$/i.test(tags.indoor ?? "")) ||
    (tags.leisure === "playground" && /^(yes)$/i.test(tags.covered ?? "")) ||
    /屋内|室内|インドア|子育てひろば|子育て支援センター|つどいの広場|ボーネルンド|キドキド|ファンタジーキッズリゾート|Kid's US|Kids US|ピュアキッズ|リトルプラネット|トンデミ|ASOBooN|CULAFUL|あそびパーク|こどもっちパーク|遊キッズ/i.test(
      name,
    )
  );
}

function isLikelyStandaloneIndoorPlay(element) {
  ...
  if (!hasIndoorEvidence(tags, name)) return false;
...
function toPlaceInput(element) {
  ...
  return {
    ...
    shortDescription: `${name}として公開地図データに登録されている屋内遊び場です。来館前に公式情報を確認してください。`,
...
function evidenceScore(element) {
  ...
  if (tags.leisure === "indoor_play") score += 6;
  if (tags.indoor === "yes" || tags.indoor === "only") score += 5;
...
(
  nwr["leisure"="indoor_play"](area.prefecture);
  nwr["leisure"="playground"]["indoor"](area.prefecture);
  nwr["leisure"="playground"]["covered"="yes"](area.prefecture);
  nwr["name"~"屋内遊び場|室内遊び場|屋内遊戯|室内遊戯|屋内広場|室内広場|キッズランド|キッズパーク|キッズスペース|キッズプラザ|キッズリゾート|こどもひろば|子どもひろば|子供ひろば|親子ひろば|子育てひろば|子育て支援センター|つどいの広場|ファンタジーキッズリゾート|Kid's US|Kids US|ピュアキッズ|ボーネルンド|あそびのせかい|キドキド|あそびパーク|リトルプラネット|トンデミ|CULAFUL|ASOBooN|こどもっちパーク|遊キッズ"](area.prefecture);
);
out center tags;`;
...
async function collectFromElements(elements) {
  ...
  for (const element of elements) {
    ...
    if (!name || !Number.isFinite(latitude) || !Number.isFinite(longitude))
      continue;
    if (!isLikelyStandaloneIndoorPlay(element)) continue;
...
async function collect(prefectureCodes = PREFECTURE_CODES) {
  ...
  for (const prefectureCode of prefectureCodes) {
    ...
    console.log(
      `Fetched ${prefectureElements.length} indoor-play candidates from ${prefectureCode}`,
    );
...
console.log(
  `Wrote ${places.length} generated indoor-play places to ${args.output}`,
);
