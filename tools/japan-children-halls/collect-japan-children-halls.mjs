...
const DEFAULT_OUTPUT = "apps/api/src/data/places/generated-children-halls.ts";
const EXISTING_INPUT = "apps/api/src/data/places/children-halls.ts";
...
function slugify(input) {
  ...
  return `osm-children-hall-${hash.toString(16).padStart(8, "0")}`;
}
...
async function getExistingPlaceNames(inputPath = EXISTING_INPUT) {
  try {
    const source = await readFile(inputPath, "utf8");
    return new Set(
      [...source.matchAll(/\bname:\s*'([^']+)'/g)].map((match) =>
        canonicalName(match[1]).normalize("NFKC"),
      ),
    );
  } catch (error) {
...
function isLikelyStandaloneChildrenHall(element, existingNames) {
  ...
  const hasPublicEvidence =
    ...
    tags.community_centre === "youth_centre";

  return (
    hasPublicEvidence ||
    /児童館|児童センター|児童会館|こども館|子ども館/.test(name)
  );
}
...
function toPlaceInput(element) {
  ...
  return {
    ...
    shortDescription: `${name}として公開地図データに登録されている児童館です。来館前に公式情報を確認してください。`,
    ...
    provenance: [
      {
        type: "open-data",
        name: "OpenStreetMap Overpass API children hall candidates",
        url: osmUrl,
...
function evidenceScore(element) {
  ...
  if (tags.amenity === "community_centre") score += 3;
...
/**
 * Generated from OpenStreetMap Overpass API children hall candidates.
 * Regenerate with:
 *   node tools/japan-children-halls/collect-japan-children-halls.mjs --output=apps/api/src/data/places/generated-children-halls.ts
 */
export const generatedChildrenHallPlaces: PlaceInput[] = [
${entries}
];
`;
}
...
(
  nwr["name"~"児童館|児童センター|児童会館|児童文化センター|こども館|子ども館|こどもセンター|子どもセンター|こども文化センター|子ども文化センター|青少年センター"](area.prefecture);
);
out center tags;`;
...
async function collectFromElements(elements) {
  ...
  for (const element of elements) {
    ...
    if (!name || !Number.isFinite(latitude) || !Number.isFinite(longitude))
      continue;
    if (!isLikelyStandaloneChildrenHall(element, existingNames)) continue;
...
async function collect(prefectureCodes = PREFECTURE_CODES) {
  ...
  for (const prefectureCode of prefectureCodes) {
    ...
    console.log(
      `Fetched ${prefectureElements.length} children-hall candidates from ${prefectureCode}`,
    );
...
console.log(
  `Wrote ${places.length} generated children-hall places to ${args.output}`,
);
