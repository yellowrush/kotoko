...
export function PlaceReservationSection({
  place,
}: PlaceReservationSectionProps) {
  ...
  return (
    <div className="kodoko-panel mt-4 p-3">
      <h3 className="text-sm font-semibold text-gray-700">
        {t('places.reservationTitle')}
      </h3>
      <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
        <span className="kodoko-badge rounded-full border border-amber-300 bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-800">
          {t(`places.reservationModes.${reservation.mode}`)}
        </span>
        {reservation.note && (
          <span className="text-gray-500">{reservation.note}</span>
        )}
      </div>
