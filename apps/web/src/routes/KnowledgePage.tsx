...
export function KnowledgePage() {
  ...
  const ageMonthsList = useMemo(
    () => children.map((child) => calculateAgeMonths(child.birthDate)),
    [children],
  );
  ...
  const visible = useMemo(() => {
    ...
    if (!hasChildren || view === 'all')
      return sortKnowledgeByRead(list, readIds);
    if (view === 'upcoming') {
      return sortKnowledgeByRead(
        filterUpcomingKnowledgeByAges(list, ageMonthsList),
        readIds,
      );
    }
    return sortKnowledgeByRead(
      filterKnowledgeByAges(list, ageMonthsList),
      readIds,
    );
  ...
  return (
    <div>
      ...
      {!isError && !hasChildren && (
        <p className="mb-3 text-sm text-gray-500">{t('knowledge.noChild')}</p>
      )}
      ...
      {!isError && !isLoading && (
        <div className="kodoko-segmented mb-4 flex p-1 text-sm">
          {(['forChild', 'upcoming', 'all'] as const).map((option) => {
            const activeOption =
              !hasChildren && option !== 'all' ? false : view === option;
            return (
              <button
                key={option}
                type="button"
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-2 text-sm text-gray-500">
                    {t('knowledge.ageRange')}: {item.minAgeMonths}~
                    {item.maxAgeMonths} {t('places.monthsUnit')}
                  </p>
                </div>
              </Link>
            </li>
